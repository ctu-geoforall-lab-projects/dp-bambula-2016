...
% Place nodes with matrix nodes
 \matrix[matrix of nodes, column sep=1cm, row sep=1cm]{%
  ...
  \node [block, xshift=2cm] (do_delete) {Delete old rows in the main database};\\
  \node [io, xshift=2cm] (find_ids) {Get IDs of new features};\\  
  \node [loop, xshift=2cm] (loop_ids) {For all IDs};\\  
  \node [block, left of=loop_ids, xshift=-2cm] (do_insert) {Insert new feature into main database};\\  
  \node [empty] (null_stop){};\\
...
% Draw edges
  ...
  \path [line] (loop_ids) -| node[anchor=south] [near start] {ID} (do_insert);
  \path [line] (do_insert) |- ([yshift=-3cm]loop_ids.south) -- (loop_ids);  
