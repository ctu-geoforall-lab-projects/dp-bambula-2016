...
%% ano, podle informaci na https://www.arcdata.cz/sluzby-a-podpora-zakazniku/sluzby/software-arcdata
...
%%% ML: mozna by stalo za to, zduraznit, ze jako jediny z popisovanych
%%% nastroju je open source (btw, nenasel jsi krome GDAL jeste neco
%%% dalsiho, vim, ze neco meli v Plzni pro prevod do PG)
%% akorat https://ojs.cvut.cz/ojs/index.php/gi/article/viewFile/gi.2.14/2592
%% ale neni zde informace o soucasnem stavu software
...
%%% ML: je otazka, zda za kazdym odstavce uvadet s tejnou citaci,
%%% lepsi je na uvod veta typu "Text teto kapitoly prevazne cerpa z
%%% ...." (coz plati pro celou kapitolu 3)
%% upraveno
...
datových skupinách se exportují pouze změnové věty.
...
pouze jeden záznam s~jedním ID.
...
pomocí tlačítek \uv{Vpřed} a~\uv{Zpět}.
...
%%% ML: Tato operace je k dispozici ?
%% upraveno
...
%%% ML: nepresne, spise zobrazovat v mapovem okne
%% upraveno
...
%%% ML: opet nevhodna ich forma, ale je v cele praci i kdyz vetsinou
%%% pouze v uvodni castech kapitol
%% upraveno
...
%%% ML: tady bych chtelo doplnit, ze qgis umoznuje ze vzdalenych
%%% repozitaru instalovat pouze python pluginy (mas to naznaceno v
%%% zaveru tohoto textu, ale ne primo), neumoznuje distribuci C++
%%% pluginu, z textu to neni zrejme
...
%%% ML: ani zkuseni uzivatele nejsou schopni casto kompilovat
...
%%% ML: zasadnim bodem teto prace je implementace novych
%%% funkcionalit...
%% upraveno
Přepis zásuvného modulu do jazyka Python je důležitým bodem této
%%% ML: tady opet ich forma, dale v textu ne
%% upraveno
...
%%% ML: tohle by spis patrilo do priloh, ale klidne to tu nech
...
\begin{figure}[htb]
\centering
\includegraphics[width=.7\textwidth]{images/qgis_plugin_vyhledani.png}
...
%%% ML: ty zmenu uz jsou nahrane v SVN vetvi (trunk) teto
%%% verze. Stacilo by napsat, ze prehled zmen popisuje prilozeny
%%% patch.
%% upraveno
Veškeré provedené změny na ovladači VFK jsou dostupné v~knihovně GDAL
od verze 2.2.0. Přehled provedených změn popisuje \uv{patch},
...
Díky změnám provedeným na VFK ovladači knihovny GDAL, bylo možné přidat
...
zdroj (viz obr. \ref{l_plugin_novy_vzhled} prvek č. 1) volbu \uv{Soubor}.
...
vstupního zdroje z~hodnoty \uv{Soubor} na hodnotu \uv{Adresář}. Následně je 
změněn vzhled \uv{widgetu} pro načítání, viz obr. \ref{l_cteni_z_adresare}.
...
\uv{stav} pro stavová data, nebo \uv{zmeny} pro data obsahující změnové věty.
...
%%% ML: co prejmenovat parametry (main->input a export -> output) ?
%% prejmenovano
\newpage
...
%%% ML: to je matouci (je to vystupni DB, viz input/output)
%% upraveno
jako výstupní / \uv{output}). Databáze se změnami je poté k~této výstupní
...
%%% ML: osobne bych dal tento diagram do prilohy, potom bude text
%%% navazovat, pokud mas pocit, ze to ma byt v textu, tak to tak nech
\begin{figure}
  \centering
...
%%% ML: projit, preformulovat, je videt, ze je text jeste "zivy"
...
%%% ML: kde "zde", cely odstavec zni nevyrovnane...
...
zásuvného modulu. V~kapitole jsem se zaměřil i~na strukturu změnových vět 
VFK tak, aby mohl být vypracován hlavní 
%%% ML: spise cil (zde hodnotis naplneni cilu, ktere jsou popsany v uvodu)
bod diplomové práce -- \textbf{zpracování změnových vět}.

Na teoretickou část volně navazuje kapitola týkající se stavu
%%% ML: zkus v textu omezit "zde"
...
%%% ML: ted me napada, ze v praci nezminijes fakt, ze pripis zacal
%%% jako semestralni projekt YFSG
...
i~ostatní členové skupiny pracující na tomto tématu, viz stránky předmětu\footnote{\url{http://geo101.fsv.cvut.cz/gwiki/155YFSG_Free_software_GIS}}.
...
%%% ML: tady by bylo dobre poznamenat, ze to neni pouze tvuj vystup, v
%%% textu to mas uvedeno, tady je to ale pro transparentnost nutne
%%% uvest take (uvod a zaver prace je nejpodstatnejsi)
Poté, co byl stávající zásuvný modul přepsán, tak byl s~pomocí vedoucího 
diplomové práce umístěn do nově vzniklého
repositáře\footnote{\url{http://geo.fsv.cvut.cz/osgeorel/qgis-plugins.xml}},
který spravuje organizace OSGeoREL na ČVUT v~Praze. Přepsaný plugin je v~repositáři
...
%%% ML: prilis komplikovana veta, preformulovat
%% upraveno
...
%%% ML: prislis malo offline zdroju (knihy, clanky a pod), vetsina
%%% jsou webove stranky (online zdroje), ale s tim asi ted moc
%%% neudelame, ale zkus popremyslet, zda bys neco nemohl pridat
%% pridal jsem nektere zdroje
\clearpage
\rhead{{\rightmark}}	% vpravo název kapitoly
...
vstupního argumentu na řetězec \uv{Číslo popisné} / \uv{Číslo
  evidenční}. Metoda \texttt{druhUcastnika()} kontroluje, o~jaký
...
%%% ML: zde je odkaz na stejnou kapitolu, ve ktere je tento text
...
byla stanovena jako \uv{zástupná} pro prvek \texttt{vfkBrowser} v~okně
...
%%% ML: podobny text uz mas v tele prace vcetne podobnych screenshotu
%%% (navrhuji ponechat prilohu a kapitolu v textu prace redukovat,
%%% popis instalace pouze zde s odkazem v textu)
...
Cesta ke vstupním souborům je zadána pomocí tlačítka \uv{Procházet} v~nabídce 
\textit{Import VFK}.
...
V~témže okně je možné zvolit možnost \uv{Přepsat stávající databázi}, která
...
%%% ML: co text prace, pridej alespon nazvy adresaru
Přiložené CD obsahuje zdrojové kódy zásuvného modulu, text diplomové
práce a~\uv{patch} knihovny GDAL vytvořený oproti verzi 2.2.0. Struktura
...
\begin{minipage}{0.9\textwidth}
  ...
  .4 citace.bib.
  .4 Makefile.
  .4 stepan-bambula-dp-2016.tex.
  .4 stepan-bambula-dp-2016.pdf.
  .4 \vdots.
  .3 zadani/.
  .4 zadanidp.doc.
  .4 zadanidp.pdf.
